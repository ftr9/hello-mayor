...
const AnnouncementsComments = () => {
  ...
  useEffect(() => {
    fetchComments(
      ...
      'isFetchingComments',
      query(
        getAdminPostCommentsCollectionRef(postId),
  ...
  const submitClickHandle = async () => {
    try {
      ...
      if (comment.length > 0) {
        ...
        setSubmitting(true);
        //1) add the comments
        await addDoc(getAdminPostCommentsCollectionRef(postId), commentFormat);

        //2) increment the comments field
        await updateDoc(adminPostDocRef(postId), {
...
export default AnnouncementsComments;
